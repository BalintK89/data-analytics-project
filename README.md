#  Upcoming: Sales Performance & Data Analysis Project

This repository will host my upcoming data analytics project, scheduled for release in **August 2026**.

### Project Objectives:
1. **Data Cleaning:** Utilizing Advanced Excel and SQL to clean, filter, and structure a raw, real-world business dataset.
2. **Database Querying:** Writing complex SQL queries (JOINS, Subqueries, Aggregate functions) to extract key business metrics.
3. **Data Visualization:** Building an interactive executive dashboard in **Tableau** to showcase trends, KPIs, and regional sales performance.
